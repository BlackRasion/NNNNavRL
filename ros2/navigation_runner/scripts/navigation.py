...
class Navigation(Node):
    ...
    def __init__(self, cfg):
        ...
        self.robot_size = 0.3 # 机器人半径 (用于安全距离计算)
        ...
        self.get_logger().info(f"[导航节点]: 速度限制: {self.vel_limit}.")
        ...
        self.odom_sub = self.create_subscription(Odometry, odom_topic, self.odom_callback, 10) # odom里程计话题
        self.goal_sub = self.create_subscription(PoseStamped, '/goal_pose', self.goal_callback, 10) # goal目标点话题
        self.emergency_stop_sub = self.create_subscription(Bool, '/navigation_emergency_stop', self.safety_check_callback, 10) # safety check紧急停止话题
        ...
        # 控制器初始化
        # =========================================================================
        self.angle_controller = AnglePIDController(kp=1.0, ki=0.0, kd=0.1, dt=0.05, max_angular_velocity=1.0) # 角度 PID 控制器 (用于朝向控制)
