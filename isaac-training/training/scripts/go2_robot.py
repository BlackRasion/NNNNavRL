...
class Go2Robot(RobotBase):
    ...
    def __init__(self, name: str = "Go2Robot", cfg: RobotCfg = None) -> None:
        ...
        self.max_linear_vel = 2.0   # 最大线速度（单位：m/s）
    ...
    def _create_prim(self, prim_path: str, translation, orientation):
        ...
        返回:
            root_prim: 创建的根节点对象
        """
        root_prim = prim_utils.create_prim( # 创建根节点（Xform 类型，用于组织层级结构）
            prim_path,
            "Xform",
    ...
    def apply_action(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        参数:
            actions: 速度命令张量，形状为 [..., 3]
                    - actions[..., 0]: Vx（前进速度），范围 [-1, 1]
                    - actions[..., 1]: Vy（横向速度），范围 [-1, 1]
                    - actions[..., 2]: Vyaw（角速度），范围 [-1, 1]
        ...
        # 构造速度向量（优化：直接构造而非克隆后修改）
        # 速度向量格式：[vx, vy, vz, wx, wy, wz]
