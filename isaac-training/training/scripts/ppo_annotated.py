"""
NavRL PPO (Proximal Policy Optimization) 算法实现

该文件实现了 PPO 强化学习算法，包括：
1. 特征提取器（CNN + MLP）
...
class PPO(TensorDictModuleBase):
    ...
    继承自 TensorDictModuleBase，使用 TorchRL 的 TensorDict 进行数据传递
    ...
    def __init__(self, cfg, observation_spec, action_spec, device):
        ...
        # -------------------------------------------------------------------------
        # 输入: LiDAR 数据 [batch, channels, height, width]
        # 输出: 128 维特征向量
        # 
        # 网络结构:
        # - Conv2d(1->4): 提取低级边缘特征
        # - Conv2d(4->16): 提取中级形状特征，水平方向降采样
        # - Conv2d(16->16): 提取高级语义特征，水平和垂直降采样
        # - Flatten + Linear: 映射到 128 维特征空间
        # - LayerNorm: 归一化，稳定训练
        #
        feature_extractor_network = nn.Sequential(
            ...
            Rearrange("n c w h -> n (c w h)"),  # 展平: [N, C, H, W] -> [N, C*H*W]
        ...
        # 输出: 64 维特征向量
        # 
        # 首先展平输入，然后通过 MLP 提取特征
        dynamic_obstacle_network = nn.Sequential(
            Rearrange("n c w h -> n (c w h)"),  # 展平动态障碍物数据
            make_mlp([128, 64])                  # MLP: 128 -> 64
        ).to(self.device)
        ...
        # 将三个特征源组合:
        # - 静态障碍物特征 (LiDAR): 128 维
        # - 机器人内部状态特征 (位置、速度等): 来自 observation.state
        # - 动态障碍物特征: 64 维
        # 
        # 组合后通过 MLP 进一步处理: 256 -> 256
        self.feature_extractor = TensorDictSequential(
            # 提取 LiDAR 静态障碍物特征
            TensorDictModule(feature_extractor_network, 
                           [("agents", "observation", "lidar")],  # 输入键
                           ["_cnn_feature"]),                      # 输出键
            # 提取动态障碍物特征
            TensorDictModule(dynamic_obstacle_network, 
                           [("agents", "observation", "dynamic_obstacle")], 
                           ["_dynamic_obstacle_feature"]),
            # 拼接所有特征: CNN + State + Dynamic Obstacle
            CatTensors(["_cnn_feature", 
                       ("agents", "observation", "state"), 
                       "_dynamic_obstacle_feature"], 
                      "_feature",           # 输出特征名
                      del_keys=False),      # 保留原始特征键
            # 通过 MLP 进一步处理组合特征
        ...
        # =========================================================================
        # Actor 使用 Beta 分布建模动作概率
        # Beta 分布适合有界动作空间 (0, 1)，通过 alpha 和 beta 参数控制分布形状
        # 输出动作通过线性变换映射到实际速度范围: [-action_limit, action_limit]
        
        self.n_agents, self.action_dim = action_spec.shape  # 智能体数量和动作维度
        
        self.actor = ProbabilisticActor(
            # BetaActor: 输入特征，输出 alpha 和 beta 参数
            TensorDictModule(BetaActor(self.action_dim), ["_feature"], ["alpha", "beta"]),
            in_keys=["alpha", "beta"],                          # 分布参数输入
            out_keys=[("agents", "action_normalized")],       # 归一化动作输出 (0, 1)
            distribution_class=IndependentBeta,                  # 使用独立 Beta 分布
            return_log_prob=True                                # 返回动作对数概率（用于 PPO）
        ).to(self.device)

        # =========================================================================
        # 步骤 3: 构建 Critic 网络（价值网络）
        # =========================================================================
        # Critic 估计状态价值 V(s)，用于计算优势函数和回报
        # 输出单个标量值表示当前状态的期望累积回报
        
        self.critic = TensorDictModule(
            nn.LazyLinear(1),              # 线性层: 特征 -> 1 (状态价值)
            ["_feature"], 
            ["state_value"]                # 输出键名
        ).to(self.device)
        ...
        # Huber Loss: 结合 L1 和 L2 损失的优点
        # 对小误差使用 L2（平滑），对大误差使用 L1（鲁棒）
        # delta=10: L1 和 L2 的切换阈值
        self.critic_loss_fn = nn.HuberLoss(delta=10)

        # =========================================================================
        # 步骤 5: 优化器设置
        # =========================================================================
        # 分别为三个组件设置优化器，可以使用不同的学习率
        ...
        # 步骤 6: 网络初始化
        # =========================================================================
        # 使用虚拟输入触发 Lazy 模块的初始化
        # LazyConv2d 和 LazyLinear 需要前向传播一次才能确定权重形状
        
        dummy_input = observation_spec.zero()  # 创建零观测作为虚拟输入
        self.__call__(dummy_input)              # 触发初始化

        # 使用正交初始化网络权重
        # 正交初始化有助于保持梯度范数，加速收敛
        def init_(module):
            if isinstance(module, nn.Linear):
                nn.init.orthogonal_(module.weight, 0.01)  # 正交初始化，增益 0.01
    ...
    def __call__(self, tensordict):
        ...
        # 公式: action = 2 * normalized_action * limit - limit
    ...
    def train(self, tensordict):
        ...
        # 步骤 4: PPO 训练循环
        # =====================================================================
        # PPO 特点: 对同一批数据进行多次 epoch 训练
        # 每次 epoch 将数据分为多个 minibatch
