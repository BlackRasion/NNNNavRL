...
class PPO(TensorDictModuleBase):
    def __init__(self, cfg, observation_spec, action_spec, device, enable_ddp=False):
        """
        初始化 PPO 策略网络
    ...
    def _update(self, tensordict): # tensordict shape (batch_size, )
        ...
        # DDP 梯度同步 (可选)
        # =========================================================================
        # 在 backward() 之后、clip 之前同步梯度
        # 这确保所有 GPU 使用相同的梯度进行裁剪和更新
        if self.enable_ddp and dist.is_initialized():
            # 同步所有参数的梯度
        ...
        # 梯度裁剪 (在梯度同步之后)
        # 注意: 梯度裁剪应在同步后进行，确保所有 GPU 使用相同的裁剪结果
