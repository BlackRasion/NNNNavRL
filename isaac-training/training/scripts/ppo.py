...
from utils import ValueNorm, make_mlp, GAE, make_batch, IndependentBeta, BetaActor, vec_to_world
...
class PPO(TensorDictModuleBase):
    ...
    def train(self, tensordict):
        ...
        terminated = tensordict["next", "terminated"]          # 仅保留终止
        ...
        # 步骤 3: 计算 GAE 优势估计和回报
        # =====================================================================
        adv, ret = self.gae(rewards, terminated, values, next_values)       # GAE 提供低方差的优势估计，同时控制偏差
    ...
    def _update(self, tensordict):
        ...
        surr2 = advantage * ratio.clamp(1.-self.cfg.actor.clip_ratio, 1.+self.cfg.actor.clip_ratio) # surr2: 裁剪后的目标，限制 ratio 在 [1-ε, 1+ε] 范围内
