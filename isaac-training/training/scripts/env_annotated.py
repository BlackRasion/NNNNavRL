...
class NavigationEnv(IsaacEnv):
    ...
    def __init__(self, cfg):
        ...
        ray_caster_cfg = RayCasterCfg(
            prim_path="/World/envs/env_.*/Hummingbird_0/base_link",  # 附加到无人机基座
            offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 0.0)),       # 无偏移
            ...
            debug_vis=False,                                          # 关闭可视化调试
            mesh_prim_paths=["/World/ground"],                       # 检测地面碰撞
        )
    ...
    def _design_scene(self):
        ...
        self.max_obs_2d_height = 5.0   # 2D障碍物高度
    ...
    def _set_specs(self):
        ...
        self.done_spec = CompositeSpec({
            ...
            "terminated": DiscreteTensorSpec(2, (1,), dtype=torch.bool), # 是否终止（碰撞/到达）
    ...
    def _compute_state_and_obs(self):
        ...
        # 获取无人机根状态
        # =========================================================================
        # root_state 包含: [世界位置(3), 姿态四元数(4), 世界速度(3), 角速度(3), ...]
        ...
        # lidar.data.pos_w: LiDAR 传感器世界坐标 [num_envs, 3]
        ray_distances = (self.lidar.data.ray_hits_w - self.lidar.data.pos_w.unsqueeze(1)).norm(dim=-1)
        
        # 限制最大距离并 reshape
        self.lidar_scan = self.lidar_range - ray_distances.clamp_max(self.lidar_range).reshape(
        ...
        if (self.cfg.env_dyn.num_obstacles != 0):
            ...
            # 用于奖励计算的动态障碍物距离
            closest_dyn_obs_distance_reward = closest_dyn_obs_rpos.norm(dim=-1) - closest_dyn_obs_size[..., 0]/2.
        ...
        # 奖励计算
        # =========================================================================
        # a. 静态障碍物安全奖励: 基于 LiDAR 距离的对数
        # 距离越近奖励越低（负值越大），鼓励保持安全距离
        ...
        # c. 速度奖励: 朝向目标方向的速度分量
        vel_direction = rpos / distance.clamp_min(1e-6)  # 目标方向单位向量
        ...
        # 高于上限
        above_mask = self.drone.pos[..., 2] > (self.height_range[..., 1] + 0.2)
        penalty_height[above_mask] = ((self.drone.pos[..., 2] - self.height_range[..., 1] - 0.2)**2)[above_mask]
        # 低于下限
        below_mask = self.drone.pos[..., 2] < (self.height_range[..., 0] - 0.2)
        penalty_height[below_mask] = ((self.height_range[..., 0] - 0.2 - self.drone.pos[..., 2])**2)[below_mask]

        # f. 碰撞检测
        # LiDAR 读数接近最大值表示有障碍物非常近
